...
def my_lenet(batch_size=250, n_epochs=2000, learning_rate = 0.01, L2_reg = 0.0001, activation=T.tanh):
    ...
    L2_sqr = (layer4.W ** 2).sum() + (layer3.W ** 2).sum() + (layer2.W ** 2).sum() + (layer1.W ** 2).sum() + (layer0.W ** 2).sum()
