...
class LeNetConvPoolLayer(object):
    ...
    def __init__(self, rng, input, filter_shape, image_shape, poolsize=(2, 2)):
        ...
        # thus be broadcasted across mini-batches and feature map
        # width & height
        self.output_x = T.tanh(pooled_out_x + self.b.dimshuffle('x', 0, 'x', 'x'))
    ...
    def ob_func(self, GT):
        return T.mean((self.output-GT)**2)


class UpSampleLayer(object):
    """UpSampling Layer of a convolutional network """

    def __init__(self, input):
        self.input = input

        A = input.repeat(2, axis=2)
        B = A.repeat(2, axis=3)
        self.output = B
       
        
class DropLayer(object):
    """DropLayer of a convolutional network """
    ...
    def __init__(self, input):
        ...
        B = B > 0.3
        C = np.zeros((3,32,32), dtype=theano.config.floatX)
        C[0] = B
        C[1] = B
        C[2] = B
        C_T = shared(C, borrow=True)
       
        self.output = C_T*input       
